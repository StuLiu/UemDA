...
parser.add_argument('--config-path', type=str, default='st.dca.2vaihingen', help='config path')
# ckpts
parser.add_argument('--ckpt-model', type=str,
                    default='log/dca/2vaihingen/align/Vaihingen_best.pth', help='model ckpt from stage1')
...
def main():
    ...
    for i_iter in tqdm(range(stop_steps)):
        ...
        # loss
        loss_seg = (loss_calc([pred_s1, pred_s2], label_s, loss_fn=loss_fn, multi=True) +
                    loss_calc([pred_t1, pred_t2], label_t_hard, loss_fn=loss_fn, multi=True))
        ...
        domain_cross = CCR([pred_s1, pred_s2, feat_s],
                           ...
                           num_classes=class_num,
                           multi_layer=True)
        loss = loss_seg + source_intra + domain_cross
